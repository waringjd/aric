...
- Adjust for gender effect on HRV
...
## Vist 1 HRV measures {.allowframebreaks}
...
char_hrv_v1 %>%
  as.matrix() %>%
  xtable(., caption = "Visit 1 HRV measures") %>%
  print(., include.rownames = FALSE, include.colnames = FALSE, caption.placement = "top", size = "\\scriptsize")

# Comparison between comorbid conditions and corrected HRV
# GENDER
df %>%
  select(ID, HFc, LFc, VLFc, TPc, LF_HF, SDNNc, RMSSDc, GENDER) %>%
...
# HTN
df %>%
  select(ID, HFc, LFc, VLFc, TPc, LF_HF, SDNNc, RMSSDc, HTN) %>%
...
# CHD
df %>%
  select(ID, HFc, LFc, VLFc, TPc, LF_HF, SDNNc, RMSSDc, CHD) %>%
...
# DIABETES
df %>%
  select(ID, HFc, LFc, VLFc, TPc, LF_HF, SDNNc, RMSSDc, DIABETES) %>%
...
## Visit 4 HRV measures {.allowframebreaks}
...
char_hrv_v4 %>%
  as.matrix() %>%
  xtable(., caption = "Visit 4 HRV measures") %>%
  print(., include.rownames = FALSE, include.colnames = FALSE, caption.placement = "top", size = "\\scriptsize")
...
## Visit 2 Stress and measures {.allowframebreaks}
...
df %>% 
  select(ID, ISEL, LSNS, AP, TA, BE, SE, MAASTRICHT, SPIELBERGER, GENDER) %>%
...
# HTN
df %>% 
  select(ID, ISEL, LSNS, AP, TA, BE, SE, MAASTRICHT, SPIELBERGER, HTN) %>%
...
# CHD
df %>% 
  select(ID, ISEL, LSNS, AP, TA, BE, SE, MAASTRICHT, SPIELBERGER, CHD) %>%
...
# DIABETES
df %>% 
  select(ID, ISEL, LSNS, AP, TA, BE, SE, MAASTRICHT, SPIELBERGER, DIABETES) %>%
...
# STROKE
df %>% 
  select(ID, ISEL, LSNS, AP, TA, BE, SE, MAASTRICHT, SPIELBERGER, STROKE) %>%
...
char_stress_v4 %>%
  as.matrix() %>%
  xtable(., caption = "Visit 4 stress measures") %>%
  print(., include.rownames = FALSE, include.colnames = FALSE, caption.placement = "top", size = "\\scriptsize")
```

### Longitudinal anger comparison {.allowframebreaks}
...
## HRV from V1 to Stress at V2^[Does HRV predict stress levels in the future?]
...
full_model <- glm(ISEL ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
# Stepwise regression, using MASS package
full_model <- glm(AP ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
# Stepwise regression, using MASS package
full_model <- glm(TA ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
# Stepwise regression, using MASS package
full_model <- glm(SE ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
# Stepwise regression, using MASS package
full_model <- glm(LSNS ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
# Stepwise regression, using MASS package
full_model <- glm(MAASTRICHT ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
...
### SPIELBERGER
# Stepwise regression, using MASS package
full_model <- glm(SPIELBERGER ~ AGE_V1 + BMI + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES + HFc + LFc + VLFc + TPc + LF_HF + SDNNc + RMSSDc + PNN50, data = df)
step_model_SPIELBERGER <- stepAIC(full_model, direction = "both", trace = TRUE, k = 2)
...
stargazer(step_model_ISEL, 
          ...
          float = TRUE,
          table.placement = "H",
          title = "LM fits for Stress ~ HRV + Covar",
          align = TRUE,
          p.auto = FALSE,
          report = "vc*",
          digits = 2,
