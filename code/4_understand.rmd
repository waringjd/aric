...
char_v1 <- 
  tabular((n = 1) ~ (GENDER + RACE + DRINKER + SMOKER + HTN + CHD + DIABETES) + (AGE_V1 + BMI)*(mean + sd), data = na.omit(cs1)) 
  
  as.matrix %>%
  xtable(., caption = 'Cohort Description')  %>%
...
# ISEL
isel_hrv <-
  compareGroups(iselgrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# AP
ap_hrv <-
  compareGroups(apgrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# TA
ta_hrv <-
  compareGroups(tagrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# BE
be_hrv <-
  compareGroups(begrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# SE
se_hrv <-
  compareGroups(segrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# LSNS
lsns_hrv <-
  compareGroups(lsnsgrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# MAASTRICH
vital_hrv <-
  compareGroups(vitalgrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

# SPIELBERGER
anger_hrv <-
  compareGroups(angergrp ~ BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN, data = df) %>%
  createTable(., type = 1, show.n = FALSE, digits = 1, hide.no = c("Absent"))

```

Each measure of psychosocial stress was divided into four quartiles. These were all compared by HRV measure to examine the relationship. Of note, the ISEL subscales and the Maastrich questionnaire were of significance trends.

----

`r export2md(isel_hrv)`
...
`r export2md(lsns_hrv)`

----

`r export2md(vital_hrv)`

----

`r export2md(anger_hrv)`



### V2 Relationship Visualization
