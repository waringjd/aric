...
```{r, results='asis'}
# Data, combining ISEL with CS2
df <- left_join(scores, cs2, by = "ID")

# Comparison for ISEL
ISEL_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ ISELg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# Comparison for AP
APg_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ APg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# Comparison for TA
ta_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ TAg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# Comparison for BE
be_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ BEg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# Comparison for SE
se_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ SEg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# Comparison for LSNS
LSNS_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ LSNSg, data = na.omit(df)) %>%
  as.matrix() %>%
...
df <-
  within(score_hpb2, {
    vitalg <- ntile(MAASTRICHT, 4) %>% factor()
  })
...
vital_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ vitalg, data = na.omit(df)) %>%
  as.matrix() %>%
...
anger_v2 <-
  tabular(Format(digits = 2) * (HTN + CHD + DIABETES + STROKE) ~ SPIELBERGERg, data = na.omit(df)) %>%
  as.matrix() %>%
...
df <-
  within(score_hpc4, {
    SPIELBERGERg <- ntile(SPIELBERGER, 4) %>% factor()
  })
...
df <- z_hrv1[c("ID", "BPM", "HF", "LF", "VLF", "TP", "LF_HF", "PNN50", "RMSSD", "SDNN")] %>% gather(variable, value, -ID)
...
# Data
df <-
  gather(hrv1[c(1,23:31)], hrv, group, 2:10) %>%
  inner_join(., gather(cs1[c(1,9:11)], comorbidity, status, 2:4)) %>%
  na.omit()
...
gDMxHRV_V1 <-
  ggplot(na.omit(df[df$status == 1 & df$comorbidity == "DIABETES", ]), aes(x = status, fill = group)) +
...
gCHDxHRV_V1 <-
  ggplot(na.omit(df[df$status == 1 & df$comorbidity == "CHD", ]), aes(x = status, fill = group)) +
  facet_wrap(~hrv, scales = "fixed") +
  geom_bar(position = "dodge") +
  geom_text(stat = "count", aes(label = ..count..), vjust = -1, position = position_dodge(width = 1)) +
  labs(
...
gHTNxHRV_V1 <-
  ggplot(na.omit(df[df$status == 1 & df$comorbidity == "HTN", ]), aes(x = status, fill = group)) +
  facet_wrap(~hrv, scales = "fixed") +
  geom_bar(position = "dodge") +
  geom_text(stat = "count", aes(label = ..count..), vjust = -1, position = position_dodge(width = 1)) +
  labs(
...
df <-
  gather(hrv1[c(1,23:31)], hrv, group, 2:10) %>%
  inner_join(., gather(cs2[c(1,8:11)], comorbidity, status, 2:5)) %>%
  na.omit()
...
gSTROKExHRV_V2 <-
  ggplot(na.omit(df[df$status == 1 & df$comorbidity == "STROKE", ]), aes(x = status, fill = group)) +
  facet_wrap(~hrv, scales = "fixed") +
  geom_bar(position = "dodge") +
  geom_text(stat = "count", aes(label = ..count..), vjust = -1, position = position_dodge(width = 1)) +
  labs(
...
```{r}
# Make a long format of variables for visualization
df <- z_hrv4[c("ID", "BPM", "HF", "LF", "VLF", "TP", "LF_HF", "PNN50", "RMSSD", "SDNN")] %>% gather(variable, value, -ID)
...
df <-
  gather(hrv4[c(1,23:31)], hrv, group, 2:10) %>%
  inner_join(., gather(cs1[c(1,9:11)], comorbidity, status, 2:4)) %>%
  na.omit()
...
df <-
  gather(hrv4[c(1,23:31)], hrv, group, 2:10) %>%
  inner_join(., gather(cs4[c(1,6:7,10:11)], comorbidity, status, 2:5), by = "ID") %>%
  na.omit()
...
# ISEL
ISEL_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ ISELg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# AP
ap_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ APg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# TA
ta_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ TAg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# BE
be_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ BEg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# ISEL-SE
se_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ SEg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# LSNS
LSNS_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ LSNSg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# MAASTRICHT
vital_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ MAASTRICHTg, data = na.omit(df)) %>%
  as.matrix() %>%
...
# SPIELBERGER
anger_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ SPIELBERGERg, data = na.omit(df)) %>%
  as.matrix() %>%
...
```{r}
# Data frame
df <- inner_join(hrv4[1:10], score_hpc4, by = "ID") %>% na.omit()
...
df <-
  within(score_hpc4, {
    angerg <- ntile(SPIELBERGER, 4) %>% factor()
  }) %>%
  inner_join(hrv4, .)

# SPIELBERGER
anger_hrv <-
  tabular(Format(digits = 2) * (BPM + HF + LF + VLF + TP + PNN50 + RMSSD + SDNN) * (mean) ~ angerg, data = na.omit(df)) %>%
  as.matrix() %>%
...
df <-
  score_hpc4 %>%
  gather(psych_variable, psych_value, -ID) %>%
  merge(., gather(z_hrv4[1:10], hrv_variable, hrv_value, -ID)) %>%
  as_tibble()
...
```{r}
# Data to be used going forward
df <- inner_join(outcomes, hrv1[c(1,23:31)], by = "ID")
```
...
# Visualization
gKaplanRR <-
  survfit(Surv(AGE_DEATH, DEAD) ~ RRg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanHF <-
  survfit(Surv(AGE_DEATH, DEAD) ~ HFg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanLF <-
  survfit(Surv(AGE_DEATH, DEAD) ~ LFg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanVLF <-
  survfit(Surv(AGE_DEATH, DEAD) ~ VLFg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanTP <-
  survfit(Surv(AGE_DEATH, DEAD) ~ TPg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanPNN50 <-
  survfit(Surv(AGE_DEATH, DEAD) ~ PNN50g, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanRMSSD <-
  survfit(Surv(AGE_DEATH, DEAD) ~ RMSSDg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanSDNN <-
  survfit(Surv(AGE_DEATH, DEAD) ~ SDNNg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
```{r}
# Data
df <- inner_join(outcomes, scores, by = "ID")
```
...
# Visualization
gKaplanISEL <-
  survfit(Surv(AGE_DEATH, DEAD) ~ ISELg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanAP <-
  survfit(Surv(AGE_DEATH, DEAD) ~ APg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanTA <-
  survfit(Surv(AGE_DEATH, DEAD) ~ TAg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanBE <-
  survfit(Surv(AGE_DEATH, DEAD) ~ BEg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanSE <-
  survfit(Surv(AGE_DEATH, DEAD) ~ SEg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanMAASTRICHT <-
  survfit(Surv(AGE_DEATH, DEAD) ~ MAASTRICHTg, data = df[df$chapter == "disease of the circulatory system",]) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
# Visualization
gKaplanSPIELBERGER_v2 <-
  survfit(Surv(AGE_DEATH, DEAD) ~ SPIELBERGERg, data = df) %>%
  ggsurvplot(., 
             data = df,
             risk.table = TRUE,
             xlim = c(50, 90),
...
### Kaplan Meier curve by Spielberger groups for overall survival
...
# Kaplan Meier curve
fit <- survfit(Surv(AGE_DEATH, DEAD) ~ LF_HFg + GENDER, data = df)
ggsurvplot(fit,
           xlim = c(50,90),
           risk.table = TRUE, 
           surv.median.line = "hv",
