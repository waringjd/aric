...
codes9 <- icd9_comorbid_elix(outcomes, icd_name = "ICD9", visit_name = "ID", date_name = "DATE_DEAD") %>% icd_comorbid_mat_to_df() %>% as_tibble()
names(codes9)[1] <- "ID"

codes10 <- icd10_comorbid_elix(outcomes, icd_name = "ICD10", visit_name = "ID", date_name = "DATE_DEAD") %>% icd_comorbid_mat_to_df() %>% as_tibble()
names(codes10)[1] <- "ID"
...
# Trim codes
codes9 <- codes9[codes9$value == TRUE, ]
codes10 <- codes10[codes10$value == TRUE, ]

# Merge into large code file
codes <- rbind(codes9, codes10)
codes <- codes[c("ID", "variable")]
names(codes) <- c("ID", "DX")
...
# Add in codes
demo <- as_tibble(merge(demo, codes, by = "ID", all.x = TRUE))
...
char_v1 <- compareGroups(~ AGE_ENROLL + AGE_DEATH + GENDER + RACE + DRINKER + SMOKER + BMI + BMI_cat + HTN + CHD + DIABETES, data = df)
...
char_v2 <- compareGroups(~ AGE_V2 + DRINKER + SMOKER + BMI + BMI_cat + HTN + CHD + DIABETES + STROKE, data = df)
...
char_v4 <- compareGroups(~ AGE_V4 + DRINKER + SMOKER + BMI + BMI_cat + HTN + CHD + DIABETES + STROKE, data = df)
...
df <-
  score_hpb2 %>% gather(variable, value, -ID)
...
df <-
  score_hpc2[c("ID", "SPIELBERGER", "anger_level")] %>%
  gather(variable, value, -ID, -anger_level)

gDistributionSpielbergerV2 <-
  ggplot(df, aes(x = value, fill = factor(anger_level))) +
  geom_histogram(aes(y = ..count../sum(..count..)), binwidth = 1) +
  labs(
...
# Overall Spielberger at V2
df <-
  score_hpc4[c("ID", "SPIELBERGER", "anger_level")] %>%
  gather(variable, value, -ID, -anger_level)
...
# Correlation tests
df <- z_hrv1[-1]
correlation_plot_v1_hrv1 <- ggpairs(df)
correlation_matrix_v1_hrv1 <- corr.test(df, 
                            use = "pairwise", 
                            method = "pearson", 
                            adjust = "bonferroni",
                            ci = TRUE)
